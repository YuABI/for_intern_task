...
module HtmlTagHelper
  ...
  def remote_function(options = {})
    ...
    function << "type: '#{options[:method] || 'GET'}',"
    function << "url:  '#{options[:url] ? url_for(options[:url]) : '#'}',"
    function << "data: #{javascript_options},"

    callbacks.each { |callback, code| function << "#{callback}: #{code}, " } if callbacks.present?
    ...
    function << if options[:position]
                  ...
                  options[:update] ? "jQuery('##{options[:update]}').html(data); " : ""
                end
    if options[:front_loading]
      function << "#{loading_close(:front)};"
    else
    ...
    if options[:front_loading]
      function = "#{loading_open(:front)}#{function};"
    else
      function = "#{loading_open}#{function};" if options[:loading]
    end
    function.html_safe
  end
