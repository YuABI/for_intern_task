class UserLifeplanAssetDecorator < ApplicationDecorator
  ...
  class << self
    ...
    def cache_deposits_form_objects(user, f)
      if user.is_a?(AdminUser)
        admin_cache_deposits_form_objects(user, f)
      else
        member_cache_deposits_form_objects(user, f)
      end
    end

    def admin_cache_deposits_form_objects(user, f)
      common_cache_deposits_form_objects(f)
    end

    def member_cache_deposits_form_objects(user, f)
      common_cache_deposits_form_objects(f)
    end

    def other_assets_form_objects(user, f)
      if user.is_a?(AdminUser)
        admin_other_assets_form_objects(user, f)
      else
        member_other_assets_form_objects(user, f)
      end
    end

    def admin_other_assets_form_objects(user, f)
      common_other_assets_form_objects(f)
    end

    def member_other_assets_form_objects(user, f)
      common_other_assets_form_objects(f)
    end
    ...
    def cash_deposits_form_columns
      %i[ user_lifeplan_asset_kind cache_deposit_kind financial_institution_name store_name account_number
          reference_on amount_of_money description ]
    end

    def other_assets_form_columns
      %i[ user_lifeplan_asset_kind other_assets_kind content company_name asset_appraisal_value rate
    ...
    def common_cache_deposits_form_objects(f)
      [
        [ init_form( f, {
    ...
    def common_other_assets_form_objects(f)
      [
        [ init_form( f, {
          code: :user_lifeplan_asset_kind,
          input: f.select(:user_lifeplan_asset_kind,
